...
const addcartfn = (cartpro) => {
  if (localStorage.getItem("login")) {
    fetch(`http://localhost:3000/cart?id=${cartpro.id}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.length == 0) {
          alert("added cart")
...
const ui = (data) => {
  ...
  data.map((prodcut) => {
    ...
    let addcrtbtn = document.createElement("button")
    addcrtbtn.innerHTML = "add to cart"
    addcrtbtn.classList.add("btn", "btn-outline-primary", "fw-semibold", "col-12", "text-uppercase")
    addcrtbtn.addEventListener("click", () => addcartfn(prodcut))

    let card = document.createElement("div")
    card.classList.add("card", "border-0", "shadow", "text-enter")
    card.append(cardimg, cardtitle, rate, cardcat, margeprice,addcrtbtn)
                                    ...
                                    <div class="modal-body">
                                        <div class=" row">
                                            ...
                                            <div class="col-8">
                                              ...
                                              Min Spend ₹3,000. Max Discount Up to ₹1000.
                                              Terms & Condition</p>
                                                <a href="" id class="btn btn-outline-primary  fw-semibold col-12 text-uppercase fw-semibold">buy now</a>
                                            </div>
                                          </div>
                                    </div>
                                </div>
